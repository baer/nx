...
// Environment variables that indicate AI agent presence (priority order)
const AI_AGENT_ENV_VARS = [
  'CLAUDECODE', // Claude Code primary
  'CLAUDE_CODE', // Claude Code fallback
  'OPENCODE', // OpenAI CLI
  // Future: CURSOR_AGENT, GEMINI_CLI
] as const;

let _isAiAgent: boolean | undefined;
...
export function isAiAgent(): boolean {
  if (_isAiAgent !== undefined) {
    return _isAiAgent;
  }

  _isAiAgent = AI_AGENT_ENV_VARS.some((envVar) => {
    const value = process.env[envVar];
    // Treat any truthy value as AI agent detected
...
/**
 * Get the detected AI agent name for telemetry/logging
 */
export function getDetectedAiAgent(): string | undefined {
...
export function buildSuccessResult(options: {
  ...
  // GitHub push step first (optional)
  steps.push({
    title: 'Push to GitHub (optional)',
  ...
  const userNextSteps: UserNextSteps = {
    description: 'REQUIRED: Show the user these exact steps to complete setup',
    steps,
  };
